...
class Simon extends Component {
  ...
  startSequence(isReplay) {
    ...
    const count = isNaN(this.state.count)
      ? 1
      : isReplay
      ? this.state.count
      : this.state.count + 1;
  ...
  renderButtons() {
    ...
    return buttons.map(name => (
      <Button name={name} clickButton={this.clickButton} />
    ));
  }

  render() {
    //turn everything off but the 'on' switch
    const SimonStyle = this.state.isOn
      ? { pointerEvents: "auto" }
      : { pointerEvents: "none" };
    ...
    return (
      <div id="simon-border" style={SimonStyle}>
        ...
        <div id="center-border">
          <div id="controls">
            <h1 id="simon-title">Simon</h1>
            <div id="start-holder">
              <div id="start-button" onClick={this.startSequence}></div>
              <p className="simon-button-label">START</p>
            </div>

            <div id="strict-holder">
              <div id="strict-button"></div>
              <p className="simon-button-label">STRICT</p>
            </div>

            <div className="on-off" id="on-off-holder">
              <p id="on" className="simon-button-label">
                ON
              </p>
              <div
                className="on-off"
              ...
              <p id="off" className="simon-button-label">
                OFF
              </p>
            </div>
            ...
            <div id="counter-holder">
              ...
              <p className="simon-button-label">COUNT</p>
            </div>
          </div>
        </div>
      </div>
